...
- [X] **Task:** Select and integrate Recharts — *why:* no charting library exists yet; needed for the interactive graphs the spec calls for — *done when:* `recharts` is added as a dependency and a first chart renders real data from the fixture dataset
- [X] **Task:** Build the Dashboard layout — *why:* the spec's central business-facing view — *done when:* renders KPIs, notices (e.g. data-quality flags surfaced in business language), quick insights, and at least 2 chart types (e.g. a trend line and a category breakdown) sourced from the pipeline's business-insights output plus the underlying cleaned dataset
- [X] **Task:** Implement dashboard-wide filters — *why:* explicit requirement — "filters which affect every piece of information shown in it" — *done when:* at least one filter control (e.g. a date-range or category filter, chosen based on what the fixture dataset supports) changes the KPIs, notices, insights, and every chart simultaneously, not just one widget

#### Milestone 9.2 — Explanation
- [X] **Task:** Build the explanation prompt and section — *why:* spec calls for "a simple yet comprehensive explanation... business model, business health status, what's good, and the most important points to consider" — *done when:* the pipeline (Milestone 8.1) generates this narrative text as its own step, and the Explanation section renders it in plain, jargon-free prose, not a bulleted data dump

#### Milestone 9.3 — Business Insights rework
- [X] **Task:** Add importance to the shared insight schema — *why:* the current `InsightItem` schema (`priority?`) needs a concrete, consistently-populated importance field for this section's sort — *done when:* `schema.ts`'s business-insights `InsightGroup` items reliably populate `priority` (e.g. high/medium/low) rather than leaving it optional/unused
- [X] **Task:** Rebuild the Business Insights section as filter/sort cards — *why:* explicit UI requirement — "filter them by tags or importance, and sort them by importance" — *done when:* each insight renders as a card (title, description, importance, tags), with working tag-filter controls and an importance sort toggle; apply the "Filterable Card List Checklist" below

#### Milestone 9.4 — Questions rework
- [X] **Task:** Add importance to client questions — *why:* mirrors the insights change; questions currently only carry topic tags — *done when:* the client-questions `InsightGroup` items carry a populated importance/priority field
- [X] **Task:** Rebuild the Questions section as filter/sort cards — *why:* same requirement pattern as Business Insights — *done when:* renders per the "Filterable Card List Checklist," with tag and importance filter/sort matching Milestone 9.3's UX so the two sections feel consistent
...
**Source:** `docs/beyond-MVP.md` — Overview, Data Dictionary, Quality, and Datasets section specs under "Data" — reconciled with a live clarification from the user (2026-08-09) that resolved a real gap: the pipeline never actually *cleaned* data, it only detected and flagged issues, and the audit trail this phase already planned assumed cleaning already existed.
**Purpose:** Two things, in dependency order. First, build the LLM-informed cleaning stage the app was missing — an "Understand & Plan Cleaning" step that reads the raw data, proposes a structured cleaning plan, and a deterministic ETL engine that applies it for real (actual value rewrites, not just detection), with every change logged. Second, re-run EDA against the now-genuinely-cleaned table and migrate/extend the Data section's UI to present all of it, including the audit trail that makes the LLM's cleaning decisions verifiable rather than a black box.
**Definition of done:** Uploading a dataset runs understand → clean → EDA (on the cleaned table) → business insights → explanation → client questions, in that order; all four Data sections render under the new shell with real content; Quality includes a real, LLM-decision-traceable audit trail of every automated modification; Datasets shows a working, genuinely-different raw-vs-cleaned comparison with both downloadable.

**Two resolved decisions this phase is built around (both explicit user choices, not assumptions):**
- **Cleaning does real work.** The "cleaned" table must end up materially different from the raw parse — categorical values unified, a documented missing-value policy applied, mixed types coerced — not just typed and date-normalized as today.
- **Cleaning is LLM-informed, not purely rule-based.** The LLM's read of each column (e.g. "this looks like a status code, values should be one of X/Y/Z") drives what gets cleaned and how. Accepted, explicit tradeoffs: cleaning is no longer perfectly deterministic (the same file can clean slightly differently on a second upload, and "Re-run analysis" can shift underlying numbers, not just regenerate text), it adds one more required LLM call before any EDA numbers can be shown, and a bad LLM cleaning decision is a real correctness risk — which is exactly why the audit trail (Milestone 10.5) and raw-vs-cleaned comparison (Milestone 10.6) are load-bearing verification surfaces in this phase, not optional polish.

#### Milestone 10.1 — Understand & Plan Cleaning (new pipeline step)
**Provider:** Mistral (data-analysis call, consistent with the data dictionary/business insights/explanation steps this one runs alongside)
- [X] **Task:** Design the cleaning-plan schema — *why:* this is actionable instructions for the ETL engine (Milestone 10.2), not display content, so it doesn't fit the existing `InsightGroup` shape — needs its own zod-validated contract, using the same layered-JSON-repair parsing helpers (`src/lib/llm/client.ts`'s `parseJsonResponse`) every other structured LLM call already relies on, generalized to validate against a schema that isn't `InsightGroup` — *done when:* a schema exists covering, per affected column: a value-mapping table (for categorical unification), a missing-value policy, and a type-coercion rule, each entry carrying a human-readable reason the audit trail (Milestone 10.2) can quote verbatim
- [X] **Task:** Build a new "Plan Cleaning" LLM call — *why:* the source of the cleaning plan, informed by the same raw-data read as the data dictionary but producing a structurally different output — *done when:* given the raw structural pass (column names/types/samples) plus a first-pass quality scan (missing values, near-duplicate categoricals, mixed types — reusing `analyzeDataQuality`/`analyzeStructure` run once against the raw table) as context, returns a schema-validated cleaning plan. Kept as its own call rather than combined with the data dictionary call below — one call, one schema, one failure mode, matching every other structured-output call in this app (dictionary/KPI/insight/recommendation/question/explanation are all separate calls too, per their own `InsightType`) rather than forcing two heterogeneous shapes through one response
- [X] **Task:** Reposition Data Dictionary generation earlier in the pipeline — *why:* per the user's step-1/2 ordering ("read to understand" before "clean"), and it already reads the same column/sample context the new Plan Cleaning call needs — *done when:* the existing, unchanged `generateDataDictionary` call runs at this same early point (in parallel with the new Plan Cleaning call, both consuming the same raw structural/quality context — similar to how business insights already runs its 3 calls concurrently via `Promise.all`) instead of its old standalone position after EDA, with no loss of existing display functionality (`DataDictionaryPanel`'s tag filtering, etc.)

#### Milestone 10.2 — ETL cleaning engine & audit trail
- [X] **Task:** Build the ETL engine that applies the cleaning plan — *why:* this is where "real fixes" actually happen — deterministic application of an LLM-authored plan, not the LLM touching data directly — *done when:* given Milestone 10.1's plan and the raw table, produces a new table with categorical values unified, missing values handled per the stated policy, and mixed types coerced, without needing a second LLM call
- [X] **Task:** Validate the plan before applying it — *why:* the LLM could propose something invalid (a column that doesn't exist, a nonsensical mapping) — this must never silently corrupt data — *done when:* any plan entry that fails validation (unknown column, type mismatch) is skipped and logged as "flagged but not auto-fixed" rather than applied or crashing the pipeline
- [X] **Task:** Log every applied (and skipped) change to a structured audit trail — *why:* the core new capability this phase exists to deliver — a data analyst must be able to "understand the reason behind every modification to the raw dataset" — *done when:* each change entry records what changed, why (the plan's stated reason), and how many rows were affected, collected into the persisted record from Milestone 6.1

#### Milestone 10.3 — Reorder the pipeline around cleaning
- [X] **Task:** Move full EDA to run after cleaning — *why:* the entire point of this phase — EDA must reflect real numbers, not raw-data artifacts already fixed away — *done when:* `analyzeNumericColumns`/`analyzeDataQuality`/`analyzeDateColumns` (Milestones 3.2-3.4) run against Milestone 10.2's cleaned table, not the raw parse; the raw table's own first-pass quality scan (Milestone 10.1, needed only to build the cleaning plan) is kept separate and not confused with final EDA output
- [X] **Task:** Update `runPipeline.ts`'s step order and the surrounding hook wiring — *why:* Phase 8's pipeline was built assuming EDA already happened before any LLM step; that assumption no longer holds — *done when:* the real sequence is parse → structural pass → understand & plan cleaning → ETL clean → EDA on cleaned table → business insights/KPIs → explanation → client questions, with `useFileUpload`/`useDatasetSession` (Phases 5-8) updated to match and resume-on-reload still working correctly at each new step
- [X] **Task:** Update the loading overlay's stage-by-stage status text — *why:* Milestone 8.2's "Understanding data..."/"Cleaning data..." strings were placeholders for phases that didn't do real work yet ("Cleaning data..." was never actually reachable) — *done when:* both strings map to genuine, distinct pipeline steps (the new understand-and-plan call, then the ETL engine actually running) instead of one being dead text
- [X] **Task:** Extend `PersistedDataset` to carry the cleaning plan and audit trail — *why:* both need to survive a reload the same way every other pipeline output already does — *done when:* Milestone 6.1's IndexedDB record includes the cleaning plan and audit trail alongside the existing `llmOutputs` slots, with the same resumable status tracking

#### Milestone 10.4 — Data section UI: Overview & Data Dictionary migration
- [X] **Task:** Migrate `DataOverviewCard`/`ColumnDetailView` into the Data → Overview destination — *why:* this content already exists and is correct (Phase 5); it now reflects genuinely cleaned-data stats rather than raw-parse stats, per this phase's reordering — *done when:* Overview renders under the new shell, sourced from the post-clean `AnalysisResult`
- [X] **Task:** Migrate the Data Dictionary panel into the Data → Data Dictionary destination — *why:* existing, correct display component (Phase 4/5) — *done when:* renders Milestone 10.1's repositioned data-dictionary output under the new navigation, with its existing tag-based filtering intact

#### Milestone 10.5 — Quality + audit trail
- [X] **Task:** Migrate `DataQualitySummaryCard` into Data → Quality — *why:* existing content, new location, now showing the *remaining* quality picture after real cleaning (ideally much cleaner than the raw scan) — *done when:* renders under the new shell, sourced from the post-clean `AnalysisResult`
- [X] **Task:** Build the "how issues were managed" explanatory section — *why:* the spec's second half of this destination, and the verification surface for this whole phase's LLM-informed cleaning decisions — *done when:* renders Milestone 10.2's audit trail in plain language grouped by transformation type, with affected-row counts and the LLM's stated reason for each, positioned alongside the quality summary

#### Milestone 10.6 — Datasets (raw vs. cleaned comparison)
- [X] **Task:** Build the side-by-side comparison view — *why:* explicit spec requirement, and now a genuinely meaningful comparison instead of two near-identical tables — *done when:* renders the first 50 rows of the raw parsed table and the first 50 rows of the actually-cleaned table side by side, both sourced from the Milestone 6.1 persisted record
- [X] **Task:** Add full-dataset downloads for both versions — *why:* spec: "they're both fully downloadable from here" — *done when:* two download buttons produce a complete (not 50-row-limited) CSV of the raw and cleaned datasets respectively

**Checkpoint: Yes — mandatory, new for this phase.** Unlike the original Data-views-only scope (mechanical migrations, no checkpoint needed), this phase now reorders a shipped, tested pipeline (Phases 3 and 8) and adds a new LLM call that materially changes dataset values, not just narrative text — a real correctness risk. Confirm the pipeline reorder and audit trail against the fixture dataset's known planted issues before considering this phase closed.
